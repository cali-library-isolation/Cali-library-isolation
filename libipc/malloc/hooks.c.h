...
{
  __shm_malloc_hook = NULL;
  ptmalloc_init ();
  return __libc_shm_malloc (sz);
}
...
{
  __shm_malloc_hook = NULL;
  __shm_realloc_hook = NULL;
  ptmalloc_init ();
  return __libc_shm_realloc (ptr, sz);
}

static void *
memalign_hook_ini (size_t alignment, size_t sz, const void *caller)
{
  __shm_memalign_hook = NULL;
  ptmalloc_init ();
  return __libc_shm_memalign (alignment, sz);
}
...
/* A flag that is set by malloc_set_state, to signal that malloc checking
   ...
   user has requested it malloc_set_state just calls __shm_malloc_check_init
...
__shm_malloc_check_init(void)
{
  ...
  __shm_malloc_hook = malloc_check;
  __shm_free_hook = free_check;
  __shm_realloc_hook = realloc_check;
  __shm_memalign_hook = memalign_check;
}
      ...
      if (top_check () >= 0)
        {
          INTERNAL_SIZE_T nb;
          checked_request2size (bytes + 1, nb);
          newmem = _int_shm_realloc(&main_arena, oldp, oldsize, nb);
        }
    }
...
/* Get/set state: malloc_get_state() records the current state of all
   ...
   __shm_malloc_check_init() if possible; if malloc checking was not in
...
void *
__shm_malloc_get_state(void)
{
  struct malloc_save_state *ms;
  int i;
  mbinptr b;

  ms = (struct malloc_save_state *) __libc_shm_malloc (sizeof (*ms));
  if (!ms)
    return 0;
...
int
__shm_malloc_set_state(void *msptr)
{
